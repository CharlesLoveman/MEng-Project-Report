...
%contributions are: implementation of array memory model for CCF and implementation of choice-out-of-k nodes in the execution tree
...
To allow for accessing sub-objects, we introduce a per byte object id $obj\_id \eq obj\_pos + offset$, and create the constraint $sym\_ptr \eq obj\_id \wedge sym\_ptr + sym\_size\leq obj\_pos + obj\_size$.
...
To overcome this, we changed the execution tree to match constraints using a unique id for each code location in the LLVM IR. We modified how CCF
...
\begin{figure}
    \centering
    \includegraphics{successful_tests.png}
...
Figure \ref{fig:successful-tests} shows the results of executing the synthetic test suite with the original version of CCF, modified to include some bug fixes, CCF with choice-out-of-k nodes implemented but using a behaviourless dummy memory model, and CCF with choice-out-of-k nodes and the array memory model implementation. We can see that the original version of CCF passes 17 out of 49 tests, corresponding to the tests that ensure no error is reported for correct program behaviour. This is the expected result as CCF does not use a memory model implementation so it should not produce any errors. With the implementation of the array memory model, the ability to detect memory bugs is greatly increased and CCF is able to correctly produce error messages for all tests that contain errors. This shows that the implementation of the array memory model matches the design of the array memory model described earlier.

\begin{figure}
    \centering
    \includegraphics{switch-compile-times.png}
    \caption{The time taken to compile \ref{LargeSwitch} using the original version of CCF and CCF with the array memory model and choice-out-of-k nodes.}
...
int main(void) {
    ...
    u_int16_t condition = 0;
    for (u_int8_t i = 0; i < 8; ++i) {
        condition |= ((u_int16_t) (inp[0] & (((u_int8_t) 1) << i))) << (2 * i);
    }
    switch(condition) {
    case 0:
        return 0;
...
\end{lstlisting}
    \caption{Large switch statement with 750 cases.}
    \label{LargeSwitch}
\end{figure}
